defmodule Project42Web.TweetChannel do
  ...
  #update the socket record
  # USE THIS TO UPDATE ETS TABLE AND FIND TARGET
   #userstruct=:ets.match(:users, {payload["name"], :"$3"})
  ...
  def handle_in("addSubscriber", payload, socket) do
    ...
    def handle_in("searchHashTag", payload, socket) do
      ...
      #IO.inspect(Project42.Server.searchHashtags(hashtag))
  ...
  def handle_in("shout", payload, socket) do
   ...
   #recieve tweeet break down into hash tags/ mentions
    #update server ets
    tweet=payload["body"]
    IO.inspect(tweet)
  ...
  #  userstruct=:ets.match(:users, {user, :"$3"})
  #  userstruct=Enum.at(List.flatten(userstruct),0)
